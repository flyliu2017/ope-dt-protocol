...
ax2 = ax1.twinx()
ax2.set_ylabel('Time for HE (s)', fontdict=font_dict)
# plt.figure(figsize=(12, 8))
# plt.tick_params(labelright=True)

width = 12

for j, i in enumerate(y[:-1]):
    ax1.bar(np.array(x) + j * width, i['ys'], width=width, label=i['label'])
ax2.bar(np.array(x) + 2 * width, [_i / 1000 for _i in y[-1]['ys']], width=width, label=y[-1]['label'], hatch='/', color='g')
